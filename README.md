...
[chasolver](https://chasolver.org) scanned billions of Lichess games and found **201,060** games decided wrongly in exactly this way. Chess LUX reaches the correct verdict, a draw, in **201,047** of them. In the remaining 13 it cannot find a proof and errs on the side of caution by awarding the win. These are not wrong verdicts but undetected draws: the game ends in a win, exactly as it does today. Amounting to roughly 1 in 15,500 of the wrongly decided games, these 13 positions are listed in the technical appendix as targets still to conquer. Since the Lichess database of standard rated games contains more than 7 billion games, the verdicts Chess LUX misses come to fewer than 2 per billion games, or less than 1 in every 500 million.
...
3. **Search:** Every move for both sides is tried up to a depth of 15 plies, and the search stops after at most 20,000 nodes. Every line must be closed by a proof: material, a locked structure, stalemate, a repeated position or the 75-move limit.
...
| Chess LUX (+ bishops, about 1 KB) | 13 | 99.97% |
| difference | 3,554 | 7.03 percentage points |
...
| file | expected |
|---|---|
| `chasolver-data-until-08-2026.csv` | 201,047 correct draws, 13 without a verdict, 0 contradictions |
| `chasolver-blocked-until-08-2026.csv` | 50,513 correct draws, the same 13 without a verdict, 0 contradictions |
...
Large files can take a few minutes to process. To try a single position, simply paste its FEN into the box on the page.
...
<details>
<summary><b>Search X: flag fall and resignation</b></summary>

If `X(g,d)` returns true, `g`'s opponent cannot mate. A node is proven in the following order:

1. `H(g)`, the material shortcut;
2. `Xs[b+t+e]>=d`, meaning the same position has already been examined to at least this depth;
3. `J(2-g)`, the one-sided lock detector that asks only about the opponent's winning chances.
...
- **Budget and depth:** If either runs out, the branch returns false and the verdict defaults to a win. The game history (fivefold repetition) does not enter the flag search.

| where | budget | depth |
|---|---|---|
| `index.html` | 20,000 nodes | 15 plies |
| `F` in the embedded engine | 50,000 nodes | 9 plies |
| benchmarking page | adjustable, default 20,000 | adjustable, default 15 |

</details>

<details>
<summary><b>Lock detector J</b></summary>

**Interface.** `J(a=3, G=0)` operates on the globals `b` and `e`; returning true constitutes a certificate.
- **`a`:** Whose winning chances are queried: 2 for White, 1 for Black, 3 for both. Dead positions use 3; `X` calls `J(2-g)`.
...
- **Boxed-in pieces:** A knight, rook or queen whose neighborhood opens up, or which the opponent can reach, records its escape in `h0`; the first escape fails that level.

In 4x the loop stops as soon as the `k2` checksum (`C+Z+K+L+r+q-P-Q+R+c`) stops changing (at most 769 iterations; never more than 69 in fuzzing). 1x always runs 768 iterations.

**Lock conditions.**
- no promotion;
- no pawn captures and no pawn checks;
- the king assumed frozen really is immobile;
- no piece has escaped its box (`!h0`);
...
**Closed soundness holes.** The old `J` issued false certificates in the positions below; today `J(3)/J(2)/J(1)` returns:
...
**Origins.** The core of `J` is FideLite's king-and-pawn lock detector. That core was fuzzed with 5.8 million unique positions and 2,405,996 chasolver queries without turning up a single counterexample. All three known holes lay in the bishop layer.
...
| measurement | result |
|---|---|
| chasolver data: proven draws | 201,047 / 201,060 (99.9935%) |
| locked-structure class: proven draws | 50,513 / 50,526 (99.97%) |
| no verdict | 13; all in the locked-structure class, and in each the search runs out of depth |
...
- **`J`:** Mutation-based fuzzing on about 375,000 locked positions yielded bit-for-bit identical verdicts from the old and the streamlined engine.

**Speed**
- **Real games:** On a 10% sample of the chasolver data (20,106 positions), median 0.1 ms, maximum 60 ms.
- **Hardest position:** `B6b/pr6/8/8/8/4p1pp/P3Pp1p/2b2K1k b - -`. It uses 19,971 nodes and returns the correct verdict (WT); about 0.4 s in a browser on a mid-range laptop.
- **What drives the time:** The cost per node. Each node involves legal move generation, a board copy and a call to `J`.
...
<details>
<summary><b>Thirteen positions left to conquer, roadmap and verification queue</b></summary>

These 13 positions are not wrong verdicts but draws that have yet to be detected. In each of them the search spends 15–30 nodes before hitting the depth limit; no position along the line can be certified by the material or lock analysis.
...
| f6c1lu7R | White | 70 | `8/8/7p/5p1P/5p1K/5Pp1/6P1/1k6 w - - 70 83` |
...
**Roadmap**
1. Teach the lock analysis to recognize these 13 structures.
...
7. **No completeness regression:** At least 201,047 correct draws on the chasolver data and at most 673 missed draws on `chasolver-positions.txt`.
...
const mirror = (x) => { const y = x.replace(/^X=/, 'Xd=').split('Xs[').join('Xt[').replace(',X(g,d-1)', ',Xd(g,d-1)'); const k = y.lastIndexOf('&&'); return y.slice(0, k + 2) + '(' + y.slice(k + 2, y.length - 1) + '||(XM=1,0)))'; };
const XD = chunks(SRC).filter((c) => c.startsWith('X=')).map(mirror)[0];
const ctx = { Math, BigInt, performance, console }; ctx.window = ctx; vm.createContext(ctx);
vm.runInContext(blocks[2], ctx);
vm.runInContext(SRC + ';' + XD + ';XM=0;Xt={};Xs={};Xm=0', ctx);
ctx.judge('8/8/4k3/3p2p1/1p1P1pP1/1P3P2/8/3K4 b - - 47 92', 15, 20000).code;   // 'DP'
```
...
[chasolver](https://chasolver.org), Lichess'in milyarlarca oyununu tarayarak bu şekilde haksız sonuçlanmış **201.060** oyun buldu. Chess LUX bunların **201.047'sinde** doğru hükmü, yani beraberliği veriyor. Kalan 13 pozisyonda kanıt bulamıyor ve güvenli tarafta kalarak galibiyet veriyor. Bunlar yanlış hüküm değil, yakalanamayan beraberlikler: oyun, bugünkü uygulamada olduğu gibi galibiyetle sonuçlanıyor. Haksız sonuçlanan oyunların yaklaşık 15.500'de 1'i olan bu 13 pozisyon, fethedilecek hedefler olarak teknik ekte listeleniyor. Lichess'in standart dereceli oyun veritabanı 7 milyarı aşkın oyun içeriyor; buna göre Chess LUX'ın kaçırdığı hükümler milyar oyunda 2'nin, yani her 500 milyon oyunda 1'in altında kalıyor.
...
3. **Arama:** İki tarafın bütün hamleleri 15 yarım hamle derinliğe kadar denenir; arama en fazla 20.000 düğümde durur. Her varyantın bir kanıtla kapanması gerekir: materyal, kilitli yapı, pat, tekrar eden pozisyon ya da 75 hamle sınırı.
...
| Chess LUX (+ filler, yaklaşık 1 KB) | 13 | %99,97 |
| fark | 3.554 | 7,03 puan |
...
| dosya | beklenen |
|---|---|
| `chasolver-data-until-08-2026.csv` | 201.047 doğru beraberlik, 13 hükümsüz, 0 çelişki |
| `chasolver-blocked-until-08-2026.csv` | 50.513 doğru beraberlik, aynı 13 hükümsüz, 0 çelişki |
...
Büyük dosyaların işlenmesi birkaç dakika sürebilir. Tek bir pozisyonu denemek için FEN'i sayfadaki kutuya yapıştırmak yeterli.
...
<details>
<summary><b>Arama X: bayrak ve terk</b></summary>

`X(g,d)` doğru dönerse `g`'nin rakibi mat edemez. Bir düğüm şu sırayla kanıtlanır:

1. `H(g)` materyal kısa yolu;
2. `Xs[b+t+e]>=d`, yani aynı pozisyonun en az bu derinlikte zaten incelenmiş olması;
3. `J(2-g)`, yalnız rakibin kazanma şansını soran tek taraflı kilit dedektörü.
...
- **Bütçe ve derinlik:** Biri biterse dal yanlış döner ve hüküm galibiyete düşer. Bayrak aramasına oyun geçmişi (beşli tekrar) girmez.

| yer | bütçe | derinlik |
|---|---|---|
| `index.html` | 20.000 düğüm | 15 ply |
| gömülü motorun `F`'si | 50.000 düğüm | 9 ply |
| ölçüm sayfası | ayarlanabilir, varsayılan 20.000 | ayarlanabilir, varsayılan 15 |

</details>

<details>
<summary><b>Kilit dedektörü J</b></summary>

**Arayüz.** `J(a=3, G=0)` global `b` ve `e` üzerinde çalışır; doğru dönmesi bir sertifikadır.
- **`a`:** Hangi tarafın kazanma şansı soruluyor: 2 beyazın, 1 siyahın, 3 ikisinin. Ölü pozisyon için 3 kullanılır; `X` içinden `J(2-g)` çağrılır.
...
- **Kutulu taşlar:** Komşuluğu açılan ya da rakibin ulaştığı at, kale veya vezir kaçışını `h0`'a yazar; ilk kaçışta o seviye düşer.

4x'te `k2` sağlaması (`C+Z+K+L+r+q-P-Q+R+c`) değişmeyince döngü durur (en fazla 769 tur; fuzz testlerinde 69'u geçmedi). 1x her zaman 768 tur döner.

**Kilit koşulları.**
- terfi yok;
- piyon alımı ve piyonla şah çekme yok;
- donmuş varsayılan şah gerçekten hareketsiz;
- hiçbir taş kutudan çıkmadı (`!h0`);
...
**Kapatılmış açıklar.** Aşağıdaki pozisyonlarda eski `J` yanlış sertifika veriyordu; bugün `J(3)/J(2)/J(1)` şunları döndürür:
...
**Köken.** `J`'nin çekirdeği FideLite'taki şah+piyon kilit dedektörü. Bu çekirdek 5,8 milyon benzersiz pozisyon ve 2.405.996 chasolver sorgusuyla fuzz testinden geçirildi; karşı örnek çıkmadı. Bilinen üç açığın üçü de fil katmanındaydı.
...
| ölçüm | sonuç |
|---|---|
| chasolver verisi: kanıtlanan beraberlik | 201.047 / 201.060 (%99,9935) |
| kilitli yapı sınıfı: kanıtlanan beraberlik | 50.513 / 50.526 (%99,97) |
| hükümsüz | 13; hepsi kilitli yapı sınıfında ve hepsinde arama derinliği yetmiyor |
...
- **`J`:** Mutasyonlu fuzz testinde yaklaşık 375 bin kilitli pozisyonda eski ve sadeleştirilmiş motor bit bit aynı hükmü verdi.

**Hız**
- **Gerçek oyunlar:** chasolver verisinin %10'luk örnekleminde (20.106 pozisyon) medyan 0,1 ms, en uzun 60 ms.
- **En zor pozisyon:** `B6b/pr6/8/8/8/4p1pp/P3Pp1p/2b2K1k b - -`. 19.971 düğüm kullanıyor ve doğru hükmü (WT) veriyor; orta sınıf bir dizüstü bilgisayarda, tarayıcıda yaklaşık 0,4 s.
- **Süreyi belirleyen:** Düğüm başına maliyet. Her düğümde legal hamle üretimi, tahta kopyası ve `J` çağrısı var.
...
<details>
<summary><b>Fethedilecek 13 pozisyon, yol haritası ve doğrulama kuyruğu</b></summary>

Bu 13 pozisyon yanlış hüküm değil, henüz yakalanamayan beraberlikler. Hepsinde arama 15–30 düğüm harcayıp derinlik sınırına takılıyor; varyant üzerindeki hiçbir pozisyon materyal ya da kilit analiziyle sertifikalanamıyor.
...
| f6c1lu7R | beyaz | 70 | `8/8/7p/5p1P/5p1K/5Pp1/6P1/1k6 w - - 70 83` |
...
**Yol haritası**
1. Kilit analizinin bu 13 yapıyı tanıması.
...
7. **Completeness gerilemesi yok:** chasolver verisinde doğru beraberlik ≥ 201.047; `chasolver-positions.txt`'te kaçan beraberlik ≤ 673 kalmalı.
